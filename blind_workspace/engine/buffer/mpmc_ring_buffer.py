"""Power-of-Two Lock-Free MPMC Ring Buffer Engine."""
...
class MPMCRingBuffer:
    ...
    def acquire_lease(self, stream_id: int, seq_no: int, priority: int, current_tick: int) -> int:
        ...
        # If no available slot, try preemption
        if target_idx == -1:
        ...
        # If still saturated, apply backpressure policy
        if target_idx == -1:
        ...
        slot.allocate_lease(lease_id, stream_id, seq_no, priority, current_tick + LEASE_DURATION_TICKS)
        self.active_leases[lease_id] = target_idx
        return lease_id
    ...
    def commit_lease(self, lease_id: int, payload: bytes) -> bool:
        ...
        if not slot.is_uncommitted_lease or slot.current_lease_id != lease_id:
            raise SlotStateViolationError(f"Slot {slot_idx} lease was preempted or superseded before commit")
